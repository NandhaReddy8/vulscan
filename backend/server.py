...
import csv
import json
from database import save_scan_request, get_scan_requests
...
@app.route("/api/results/<path:url>", methods=["GET"])
def get_scan_results(url):
    """Fetch the latest scan results for a given target URL"""
    sanitized_url = url.replace("://", "_").replace("/", "_")
    json_filename = f"{RESULTS_DIR}/{sanitized_url}.json"

    if not os.path.exists(json_filename):
        return jsonify({"error": "Scan results not available yet."}), 404
